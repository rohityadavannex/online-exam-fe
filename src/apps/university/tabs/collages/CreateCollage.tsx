...
const CreateCollage = () => {
  useSetActiveTab(TAB_NAMES.COLLAGES);
  ...
  const {
    ...
    error: isUpdateError,
  } = useUpdateCollage({ collageId: Number(collageId) });

  const { isLoading: isGetInfoLoading, data: response } = useGetCollageInfo({
    collageId: Number(collageId),
  });
      ...
      <div className="flex flex-col bg-white rounded-md py-[42px] px-[34px]">
        <form onSubmit={handleSubmit} className="flex flex-col" noValidate>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Input
              type="number"
              label="Collage Contact No."
              placeholder="Enter Collage Contact No."
